...
public class XPathSelectors : BaseTest
{
    ...
    public void BasicXPathSelectors()
    {
        ...
        Assert.That(Driver.FindElement(By.XPath("//div[text() = 'All Projects']"))
            .Displayed); //отличие XPath от css - есть быстрый поиск текста
    ...
    public void AxesXPathTest()
    {
        ...
        Assert.That(Driver.FindElement(By.XPath("//*[@class='dialog-title']/following-sibling::form"))
            .Displayed); //если несколько, берет тот уровень, где больше. Найденные элементы должны быть на одном уровне с элементом, которым мы "режем" документ
